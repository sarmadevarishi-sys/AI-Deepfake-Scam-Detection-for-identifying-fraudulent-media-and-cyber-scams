...
_pipe1 = None
_pipe2 = None
M1 = "buildborderless/CommunityForensics-DeepfakeDet-ViT"
...
def _g1():
    global _pipe1
    if _pipe1 is None: _pipe1 = _load(M1, "buildborderless DeepFake-Detector")
    return _pipe1

def _g2():
    global _pipe2
    if _pipe2 is None: _pipe2 = _load(M2, "Organika-SDXL-Detector")
    return _pipe2
...
def _score(res):
    ...
    if "deepfake" in scores:
        return round(scores["deepfake"] * 100)

    if "realism" in scores:
        return round((1 - scores["realism"]) * 100)

    # Generic fallback
    for r in res:
        lbl = r["label"].lower()

        if any(k in lbl for k in ["fake", "ai", "artificial", "generated", "synthetic"]):
            return round(r["score"] * 100)

        if any(k in lbl for k in ["real", "authentic", "genuine", "human"]):
...
def analyze_image(fb: bytes) -> dict:
    ...
    if p:
        try:
            ...
            print (r) 
            s = _score(r)
            rows.append((s, 0.10, M1)); raw["m1"] = s
        except Exception as e:
            print(f"[IM] m1 err {e}")
    ...
    if p:
        try:
            ...
            print (r) 
            s = _score(r)
            rows.append((s, 0.88, M2)); raw["m2"] = s
        except Exception as e:
            print(f"[IM] m2 err {e}")

    es, ed = _exif(img)
    rows.append((es, 0.02, "EXIF")); raw["exif"] = ed
